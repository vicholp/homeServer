...
function beet_showResults(results){
  ...
  let list = document.querySelector("#cardBody-songs");
  list.innerHTML = "";
  ...
  for (const [i, song] of results.entries()){
    ...
    t.querySelector(".list-song-play").setAttribute('onclick',`beet_play('searchResults', ${i})`);
    t.querySelector(".list-song-info").setAttribute('onclick',`beet_info('searchResults', ${i})`);
